/**
对于l r t,任意的l<=i<=r a[i]=min(a[i],t)
输出[l,r]最大值
区间[l,r]和
*/
#include<bits/stdc++.h>
...
/*
mx[u]: 表示以节点 u 为根的子树中的最大值。
se[u]: 表示以节点 u 为根的子树中的次大值。
cn[u]: 表示以节点 u 为根的子树中最大值出现的次数。
tag[u]: 表示节点 u 的懒惰标记，用于延迟更新操作。

注意：a数组的小标从1开始，查找的下标也是从1开始
*/
